...
namespace Artemis.Backend.Services.ApplicationManagement
{
    public class GetApplicationsService(
         ...
         IMapper mapper,
         ILogger<GetApplicationsService> logger) : BaseService<FilterDTO>
    {
        private readonly ArtemisDbContext _context = context;
        private readonly IMapper _mapper = mapper;
        private readonly ILogger<GetApplicationsService> _logger = logger;
        ...
        public override async Task<ResultNotifier> ExecuteAsync(FilterDTO filter)
        {
            try
            {
                ...
                var paginationResult = new PaginationDTO<ApplicationDTO>
                {
            ...
            catch (Exception ex)
            {
                ...
                return ResultNotifier.Failure("Error retrieving applications");
            }
        }

        private static IQueryable<Application> ApplyFilters(IQueryable<Application> query, FilterDTO? filter)
        {
            if (!string.IsNullOrEmpty(filter?.SearchTerm))
            {
                var searchTerm = filter.SearchTerm.ToLower();
                query = query.Where(a =>
                    a.Title.Contains(searchTerm, StringComparison.CurrentCultureIgnoreCase) ||
                    a.Station.Contains(searchTerm, StringComparison.CurrentCultureIgnoreCase) ||
                    a.Route.Contains(searchTerm, StringComparison.CurrentCultureIgnoreCase) ||
                    a.ApplicationType.Contains(searchTerm, StringComparison.CurrentCultureIgnoreCase)
                );
            }
